...
// Show 'Thanks for visiting' at bottom of page only when scrolled to end
(function() {
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function checkScroll() {
      ...
      if (scrollPos >= docHeight - 10) {
        if (!shown) {
          ...
          // Force a reflow so opacity transition works
          void thanks.offsetWidth;
          thanks.style.opacity = 1;
          // Add glow effect after fade-in completes (10s)
          setTimeout(() => {
            if (thanks.style.opacity == '1') {
              thanks.classList.add('glow-effect');
            }
          }, 10000);
          shown = true;
        }
        ...
        if (shown) {
          ...
          thanks.classList.remove('glow-effect');
          setTimeout(() => { thanks.style.display = 'none'; }, 10000);
          shown = false;
        }
      }
    }
